...
Some lines in the examples are rather long and needed to be wrapped around for reasons of readability. If a piece of a line originally belongs to the line above, it is indicated with the symbol \unicode{↪} at the beginning of the line. This is only done for display purposes and should not be replicated in the transcription.

\subsection{XML Notation}
\label{sec:xml-notation}

Attributes of XML elements are prefixed by ``@'' (e.g. \xmex{@n} or \xmex{@type=”head”}).
...
Since the expected result is a well-formed XML file, at least the charaters \xmex{\&} and \xmex{<} must not be written as such. The character \xmex{\&} must be typed as \xent{amp} and \xmex{<} must be written as \xmex{\&lt;} instead.
...
\begin{crossref}
For §æ§ see \sect{section characters to be typed directly}. For ligatures, e.g. {\fontspec[Ligatures=Rare]{Hoefler Text} ct}, see \sect{section latin ligatures}. \xmcl{p} marks the end of a paragraph (\sect{section paragraphs}). For spaces before and after punctuation marks see \sect{section latin punctuation}.
\end{crossref}

\begin{note}
In this example, the \xml{p} for the beginning of the paragraph is on the previous page.
\end{note}
...
\label{textblocks}
\begin{mainrule}
Type an \xms{lb} element at the beginning of each line of text of the printed page. Exceptions are the elements \xml{trailer}, \xml{head}, \xml{item}, \xml{cell}, \xml{note} and \xml{ab} if the contained text is not longer than one line.
\end{mainrule}
...
\bold{<lb/>}proijciendo lapides funda, \xent{amp} circunducentes brachium, nam funes tanto maius \\
\bold{<lb/>}pondus acquirunt, \xent{amp} manum tanto magis onerant, quanto velocius voluitur funda, \\
\bold{<lb/>}\xent{amp} incitatur motus, quod ab appetitu naturali inſito ei corpori per lineã\lwr rectampro- \\
\bold{<lb/>}grediendi procedit. Vnde fit, vt pondus circunferentiæ ipſius rotæ, tanto facilius\lwr cir- \\
\bold{<lb/>}cunuoluatur, \xent{amp} ex ſeipſo tanto longiori tempore moueatur, quanto longius\lwr diſtat à \\
...
\begin{note}
Headings (\sect{section headings}) are marked by \xmlpair{head} instead of \xmlpair{p}. Block quotations  (\sect{section block quotations}) are marked by \xmlpair{quote} instead of \xmlpair{p}. The \xmlpair{p} tags should not be used in marginal notes (\sect{section marginal notes}) or footnotes (\sect{section footnotes}). For the encoding of §ct§, refer to section \ref{section latin ligatures}.
\end{note}
...
\bold{<lb/>}Impetus, \xent{amp} rapido contrarius euehor orbi.\bold{</quote>} \\
...
\begin{mainruleLessImportant}
If you can identify a paragraph as a footer, use \xmlpair{trailer}.
\end{mainruleLessImportant}
...
\bold{<lb/>}Martis An\xent{ae}ret\xent{ae} perueniam, qu\xent{ae} quidem directio circa annum milleſimum\lwr quin- \\
\bold{<lb/>}genteſimum nonageſimum ſecundum eueniet.\bold{</p>}\\
\bold{<trailer>}FINIS.\bold{</trailer>}
\end{typeLatin}
...
\bold{<lb/>}bet in aere \xent{amp} aqua.\bold{</head>}  \\
\bold{<row><cell>}\bold{<lb/>}Qualitas \bold{<lb/>} Auri. \bold{</cell><cell>} \bold{<lb/>}Grauitas Auri \bold{<lb/>}\lwr in aere.\bold{</cell><cell cols="4">} \bold{<lb/>}Grauitas Auri in aqua.\bold{</cell>}\bold{<cell><lb/>}  \lwr Miſt\~u ex Arg.<lb/> \xent{amp} ære.\bold{</cell></row>} \\
...
\bold{<row><cell>}\bold{<lb/>}Pars proportio \bold{<lb/>} nalis Auri in \bold{<lb/>} partibus. 24. \bold{</cell>}\lwr\bold{<cell cols="2"><lb/>} Differ\~etia Gra \bold{<lb/>}uitatum Auri \bold{<lb/>} in aqua.\bold{</cell></row>} \\
...
\begin{note}
If there is no running text in the columns, they may not be separate columns, but a table (\sect{section tables}). If in doubt, check the example there. Also note that in Example 1 above, the page number and the running head are not part of a column.
\end{note}
...
\bold{<item>}\bold{<lb/>}Cap. 1. \bold{<space/>} De Chorographia generatim: quid ſit, \xent{amp} quæ ad eam In-\\
\bold{<lb/>} ſtrumenta poti\xent{ſs}imùm requiſita, \bold{<ref>} pag. 1.\bold{</ref></item>} \\
...
\bold{<lb/>} Voutes, \xent{amp} où l'on raporte quelques principes tirés de la méca- \\
...
\begin{mainrule}
If a normal paragraph contains large spaces, mark them with \xms{space}.
\end{mainrule}

\begin{clarification}
Before you use \xms{space}, make sure the paragraph is not part of a table, a heading, an index or a table of contents. In some texts the spaces after periods (“.”) is slightly larger than normal spaces; do not mark this.
\end{clarification}
...
\begin{typeLatin}
\bold{<p>} \someText \\
\bold{<lb/>}extrema ad axes \bold{<space/>} angulorum. continent autem hunc propoſitiones  \\
\bold{<lb/>}ferè exiſtentes vna multa, \xent{amp} varia theoremata, \xent{amp} linearum, \xent{amp}\lwr ſuperficie-  \\
\bold{<lb/>}rum, \xent{amp} ſolidorum omnia ſimul vna demonſtratione, \xent{amp} quæ nondum de-  \\
\bold{<lb/>}monſtrata ſunt, \xent{amp} quæ \bold{<space/>} \xent{amp} in duodecimo libro horum elemento-  \\
\someText \bold{</p>}
\end{typeLatin}
\end{sampleImageSmall}
...
\begin{note}
Do not transcribe handwritten notes (see \sect{section handwritten notes}).
\end{note}
...
\bold{<lb/>}rect\xent{ae} line\xent{ae}, qu\xent{ae} ex ipſa cõparatione ſunt facta.
\bold{</p>}
\end{typeLatin}
\end{sampleImage}
...
\begin{note}
If the footnote continues on the next page, type it in the \xml{note} element of the first page anyway and do not mark the pagebreak with a \xms{pb}.
\end{note}
...
\begin{note}
Treat handwritten figures in the same way as handwritten notes (\sect{section handwritten notes}), i.e.  mark them by the milestone \xmsatt{add}{rend}{handwritten}.
\end{note}
...
\begin{mainrule}
  Where a figure occurs in the text, type \xmsatt{figure}{place}{here} on a separate line.

  If you can identify a caption that belongs to the figure, use \xmlatt{figure}{place}{here} and include the caption using \xmlpair{head}. Additional text that describes parts of the figure is marked by the \xml{ab} element and the attribute \attr{ab}{type="desc"}.

  Use another \xml{ab} element with the attribute \attr{ab}{type="var"} to record variable names and numbers.
\end{mainrule}
...
\bold{<p>}\bold{<lb/>}Si areola comprehenſa fuerit ſub rationali \xent{amp} apotome ſecunda \\
...
\bold{<lb/>}cõpreh\bs\tld{}edatur ſub ratiõali α γ, \\xent{amp} \bold{<hi rend="emph">}2\bold{</hi>} a- \\
...
\begin{crossref}
For \xms{gap} see section \ref{section characters you are unsure about}, for \xml{hi} see section \ref{sec:type-styles}.
\end{crossref}
...
\begin{clarification}
Do not use a \xml{figure} tag (see \sect{section figures}) for ornamental drop caps.
\end{clarification}
...
\bold{<lb/>}na vidiſſet quaſdã figuras geometricas delineatas exultant\bs\tld{}e l\xent{ae}titia\lwr dixiſſe: Hæc \\
\someText \bold{</p>}
\end{typeLatin}
\end{sampleImage}
...
\bold{<lb/>}titatem ſuper ipſum corpus lunare videamus, \xent{amp} eo modo terminatã quo \\
...
\begin{sampleImage}[ 1: \, Library Stamp]{librarystamp}
  \begin{typeLatin}
\notTranscribed
  \end{typeLatin}
\end{sampleImage}
...
\begin{typeLatin}
\bold{<p>} \someText
\bold{<lb/>}h. ſupponendo igitur in puncto. g. pondus, aut virtutem mouentem unius\lwr libræ, \xent{amp} in \\
\bold{<lb/>}h. duarum librarum, abſq; dubio hæ duæ uirtutes in huiuſmodi diſtantijs\lwr à centro \\
\bold{<lb/>}\xent{ae}quales inuic\bs\tld{}e er\bs\tld{}ut, ob rationes prioribus capitibus iam allatas, \xent{amp}\lwr ſtatera orizontalis \\
...
\begin{crossref}
Example 2 contains a signature and a catchword (see \sect{section catchwords and signatures}) which are to be ignored.
\end{crossref}
...
\begin{typeLatin}
\bold{<p>} \someText
\bold{<lb/>}d \xent{amp} f ad quadratum f, ergo diſiunctim erit quadratu\bold{<unclear>}m\bold{</unclear>}\lwr\bold{<gap/>} ad \\
...
\begin{typeLatin}
\bold{<p>} \someText
\bold{<lb/>}feſſore: cui acceptum feras quicquid hîc aut ad Gr\xent{ae}cum\lwr exe\bold{<unclear>}m\bold{</unclear>}plar, aut alio- \\
\bold{<lb/>}qui doctè reſtitutum uideris. Adiecimus Phænomena, S\bold{<gap/>}ecularia, Pro- \\
\bold{<lb/>}theoriam Marini, \xent{amp} Data, argumentorum ſimilitudine in\bold{<unclear>}d\bold{</unclear>}ucti.\lwr Quum\bs´\xent{q3}
\someText \bold{</p>}
\end{typeLatin}
\end{sampleImage}
...
\begin{mainrule}
If you have reason to believe that there is a mistake in the text, surround it by \xmlpair{sic}, or if the mistake is found in an attribute, use a milestone tag, \xms{sic} after the element containing the mistake. See the example below.
\end{mainrule}
...
\bold{<lb/>}drato. 4. colligemus, nempè. 1\bold{<hi rend="emph">}6\bold{</hi>}. erit\bs'q; quadratum. 25. \xent{amp}\lwr ita quadratum. \bold{<hi rend="emph">}6\bold{</hi>}. hoc eſt. \\
...
\bold{<lb/>}contextu corporis tegumentum faciet ſine \xent{ij}ſdem inſtrumentis, nec pel- \\
\someText \bold{</p>}
\end{typeLatin}
\end{sampleImage}
...
Use the element \xmlatt{hi}{rend}{emph} to mark words or whole lines in italics. Encode only up to a few lines of text this way. A whole paragraph in italics is marked by the \attr{p}{rend="emph"} in the \xml{p} tag. Also, be careful not to include punctuation marks.
  ...
  Within a larger \attr{p}{rend="emph"} structure like paragraph or table you can use \xmlpatt{hi}{rend}{emph} to mark single words in upright type (see \xml{hi rend="emph">THEON</hi} in example 2 in \sect{section figures}).
\end{mainrule}

\begin{clarification}
  This applies not only to paragraphs (see \sect{section paragraphs}), but also to tables (see \sect{section tables}), i.e. \xml{table rend="emph"}.
...
\bold{<lb/>}telligentem phantaſmata ſpeculari\bold{</hi>}, cum pr\xent{ae}terea perſpicuum ſit, diſcretum \\
...
\bold{<lb/>}tionibus, accuratiſque ſcholi\xent{is} illuſtrati: nunc quartò editi,  \\
\bold{<lb/>}ac multarum rerum acce\xent{sz}ione post primam  \\
...
\\ \\
\includegraphics[height=8mm]{uo_ligature} & \xent{uo} %tur
\end{longtable}
\end{liste}
...
\vspace{-3mm}
\begin{typeLatin}
\bold{<hi rend="emph">}acce\xent{sz}ione\bold{</hi>}
\end{typeLatin}
...
\begin{mainrule}
Words or groups of words in letter-spacing, i.e. words with extra space between each letter, are marked by \xmlpatt{hi}{rend}{sp}.
\end{mainrule}
...
\begin{mainrule}
Roots are surrounded by \xmlpair{formula}, prefixed by §√§ and the term of the root surrounded by §{ }§. If there is a number or letter above the root symbol, type it in square brackets after after the §√§, e.g. §√[3]§ (see Example 4 below).
\end{mainrule}
...
\bold{<lb/>}Cubus FG = \bold{<formula>}√\bold{\{}2 \bold{<hi rend="emph">}rx\bold{</hi>} - \bold{<hi rend="emph">}xx\bold{</hi>}\lwr\bold{\}</formula>} \bold{<g ref="\#001"/>} \bold{<formula>}√\bold{\{}2 \bold{<hi rend="emph">}rx\bold{</hi>} - \bold{<hi rend="emph">}\lwr xx\bold{</hi>}.\bold{\}}\bold{</formula>} \xent{amp} Cubus AD = \bold{<hi rend="emph">}r\bold{</hi><hi rend="sup">}3\bold{</hi>}.\lwr qua- \\
